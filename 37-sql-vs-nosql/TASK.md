...
This topic will also practice separate route files.

## Task 2

In `src/routes/database.routes.ts`, create an Express router.
...
In `src/index.ts`, create the app and mount routes:
...
```json
"dev:sql-nosql": "tsx 37-sql-vs-nosql/src/index.ts"
```
...
- Use separate route files in Express
- Mount route files using `app.use()`
